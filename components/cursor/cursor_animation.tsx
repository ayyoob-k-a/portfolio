...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function pointerPrototype(this: Pointer): void {
      this.id = -1;
      this.texcoordX = 0;
      this.texcoordY = 0;
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: Color): void {
      splatProgram.bind();
    ...
    const handleFirstMouseMove = (e: MouseEvent): void => {
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(e.clientX);
      const posY = scaleByPixelRatio(e.clientY);
      const color = generateColor();
    ...
    const handleFirstTouchStart = (e: TouchEvent): void => {
